...
def process_dates(data, sid_df, schema, logger, dateDict):
    ...
    # Decide output granularity per derived field based on how many components
    # the question exposes in dateDict:
    #   3+ components -> full date (DMY)
    #   2 components  -> month+year (MY)
    #   1 component   -> year only (Y)
    granularity = {}
    for _new_field, _components in flat_names.items():
        n = len(_components or [])
        if n >= 3:
            granularity[_new_field] = "DMY"
        elif n == 2:
            granularity[_new_field] = "MY"
        else:
            granularity[_new_field] = "Y"
    ...
    def _format_shifted(dt_obj, shift_days, new_field_name):
        """
        Apply the subject-specific shift and then truncate the output
        ...
        if not isinstance(dt_obj, datetime):
            return None
        shifted = dt_obj + timedelta(days=shift_days)
        fmt = granularity.get(new_field_name, "DMY")
        if fmt == "Y":
        ...
        for new_field, comp_paths in path_keys_dict.items():
            ...
            if stars == 0:
                ...
                if isinstance(dt, datetime):
                    value = _format_shifted(dt, shift_days, new_field)
                    _set_in_record(record, new_field, value)
                else:
                    ...
                    for base_item in top_arr:
                        ...
                        if isinstance(sub_arr, list):
                            for sub_item in sub_arr:
                                ...
                                if isinstance(dt, datetime):
                                    value = _format_shifted(dt, shift_days, new_field)
                                    _set_rel(sub_item, new_field, value)
                                else:
...
def update_schema(old_schema_path, new_schema_path, dateDict, pii_vars, section_name):
    ...
    def _derived_date_description(new_field, components):
        """
        Build a human-readable description for a derived date field, explaining
        how partial dates are completed and shifted before being stored.
        """
        n = len(components or [])
        if n >= 3:
            # Day-month-year questions
            return (
                "Derived and pseudo-anonymised full date. Day, month and year "
                "components are combined and the resulting date is shifted by a "
                "participant-specific offset. If the day component is missing it "
                "is set to 15 before shifting. If the month component is missing "
            ...
            return (
                "Derived and pseudo-anonymised month/year. Month and year "
                "components are combined into a date with day 15, then shifted "
                "by a participant-specific offset, and only the year and month "
                "of the shifted date are stored. If the month component is "
                "missing (and a year is present) the date is anchored at "
                "1 July (day 1, month 7) of that year before shifting. If the "
                "year component is missing the derived value is null."
            )
        else:
            # Year-only questions
            return (
                "Derived and pseudo-anonymised year. The date is anchored at "
                "1 July (day 1, month 7) of the reported year, shifted by a "
                "participant-specific offset, and only the year of the shifted "
                "date is stored. If the year component is missing the derived "
                "value is null."
            )
    ...
    for new_field, components in inner_dict.items():
        ...
        n_components = len(components or [])
        # Base field meta
        base_items = [
            ("name", new_field),
            ("description", _derived_date_description(new_field, components)),
            ("type", ["string", "null"]),
        ]
        # Granularity-specific constraint: full date vs month-year vs year-only
        if n_components >= 3:
            # Full DMY date: keep JSON Schema date format
            base_items.append(("format", "date"))
        elif n_components == 2:
            # Month-year: require YYYY-MM
            base_items.append(("pattern", r"^[0-9]{4}-(0[1-9]|1[0-2])$"))
        else:
